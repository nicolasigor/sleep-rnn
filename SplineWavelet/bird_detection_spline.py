...
class spline_BULBUL:
    def __init__(self, x_shape, S, N, J, Q, deterministic=0, initialization='random',
                 renormalization=theano.tensor.max, chirplet=1, aug=1, complex_=1, log_=1):
        ...
        layers.append(SplineFilter1D(
            layers[-1], N=int(N), J=int(J), Q=int(Q), S=int(S), type_='hermite', stride=1, pad='valid',
            nonlinearity=1, deterministic=deterministic, initialization=initialization, renormalization=renormalization,
            chirplet=chirplet, complex_=complex_))
...
#         layers.append(lasagne.layers.NonlinearityLayer(lasagne.layers.BatchNormLayer(layers[-1],axes=[0,1,3]),nonlinearity=lasagne.nonlinearities.rectify))
        ...
        layers.append(lasagne.layers.batch_norm(lasagne.layers.Conv2DLayer(
            layers[-1], num_filters=16, filter_size=(3, 3), nonlinearity=lasagne.nonlinearities.leaky_rectify)))
        layers.append(lasagne.layers.Pool2DLayer(layers[-1], (3, 3)))
        layers.append(lasagne.layers.batch_norm(lasagne.layers.Conv2DLayer(
            layers[-1], num_filters=16, filter_size=(3, 3), nonlinearity=lasagne.nonlinearities.leaky_rectify)))
        layers.append(lasagne.layers.Pool2DLayer(layers[-1], (3, 3)))
        layers.append(lasagne.layers.batch_norm(lasagne.layers.Conv2DLayer(
            layers[-1], num_filters=16, filter_size=(1, 3), nonlinearity=lasagne.nonlinearities.leaky_rectify)))
        layers.append(lasagne.layers.Pool2DLayer(layers[-1], (1, 3)))
        layers.append(lasagne.layers.DropoutLayer(lasagne.layers.batch_norm(lasagne.layers.Conv2DLayer(
            layers[-1], num_filters=16, filter_size=(1, 3), nonlinearity=lasagne.nonlinearities.leaky_rectify))))
        layers.append(lasagne.layers.Pool2DLayer(layers[-1], (1, 3)))
        layers.append(lasagne.layers.DropoutLayer(lasagne.layers.batch_norm(lasagne.layers.DenseLayer(
            layers[-1], 256, nonlinearity=lasagne.nonlinearities.leaky_rectify))))
        layers.append(lasagne.layers.DropoutLayer(lasagne.layers.batch_norm(lasagne.layers.DenseLayer(
            layers[-1], 32, nonlinearity=lasagne.nonlinearities.leaky_rectify))))
...
class conv_BULBUL:
    def __init__(self, x_shape, S, N, J, Q, aug=1, log_=1):
        ...
        layers.append(lasagne.layers.ReshapeLayer(layers[-1], (x_shape[0], 1, x_shape[1])))
        filter_size = int(N*2**J)
        L1 = lasagne.layers.Conv1DLayer(layers[-1], num_filters=J*Q, filter_size=int(N*2**((J*Q+1)/Q)),
                                        nonlinearity=theano.tensor.abs_)
        L2 = lasagne.layers.Conv1DLayer(layers[-1], num_filters=J*Q, filter_size=int(N*2**((J*Q+1)/Q)),
                                        nonlinearity=theano.tensor.abs_)
