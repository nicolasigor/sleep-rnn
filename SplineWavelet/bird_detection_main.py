...
# python bird_detection_main.py 10 1  0.01 'splineBULBUL' 1 'gabor'
    ...
    for n_epoch in xrange(n_epochs):
        ...
        for batch in xrange(n_batch):
            ...
            if isinstance(MODEL, BULBUL):
                x_batch = asarray([base.logfbank(
                    x, samplerate=22050, winlen=0.046, winstep=0.010, nfilt=80, nfft=1024, lowfreq=10, highfreq=11000
                ).T for x in x_batch]).astype('float32')
...
def testit(X, Y, batch_size, MODEL):
    ...
    for batch in xrange(n_batch):
        if isinstance(MODEL,BULBUL):
            x_batch = asarray([base.logfbank(
                x,samplerate=22050, winlen=0.046, winstep=0.010, nfilt=80, nfft=1024, lowfreq=10, highfreq=11000
            ).T for x in X[batch*batch_size:(batch+1)*batch_size].astype('float32')]).astype('float32')
        else:
...
csv_file = open(METADATA_FILE, 'rb')
...
for i in xrange(5):
    X_train, X_test, Y_train, Y_test = train_test_split(vstack(DATA)[p], labels[p], test_size=0.33,
                                                        stratify=labels[p], random_state=10+i)
    ...
    if MODEL_ == 'BULBUL':
        ...
        MODEL = spline_BULBUL(x_shape=(batch_size_, shape(X_train)[1]), S=S, N=N, J=J, Q=Q, deterministic=0,
                              initialization=init_, renormalization=lambda x: x.norm(2), chirplet=chirp,
                              aug=aug_, complex_=complex_, log_=log_)
    elif MODEL_ == 'GABOR':
