...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'var_reg_lagged_first_grid'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    description_str = 'v11 with lagged variability regularization'
    verbose = True
    ...
    # Grid parameters
    reg_param_list = [1e-4, 1e-3, 1e-2, 1e-1, 1]
    reg_lag_list = [6, 4, 2]
    params_list = list(itertools.product(
        reg_param_list, reg_lag_list
    ))
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for reg_param, reg_lag in params_list:
                    model_version = params[pkeys.MODEL_VERSION]

                    params[pkeys.PREDICTION_VARIABILITY_REGULARIZER] = reg_param
                    params[pkeys.PREDICTION_VARIABILITY_LAG] = reg_lag

                    folder_name = '%s_%s_lag%d_%s' % (
                        model_version,
                        type_loss,
                        reg_lag,
                        np.log10(reg_param)
