...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'false_spindles_alpha_mvp'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    contamination_min_amplitude_list = [
        0.5,
        0.6,
        0.7,
        0.8,
    ...
    contamination_frequency_range_list = [
        [9, 12]
    ]
    proba_list = [
        # 1.0,
    ...
    params_list = list(itertools.product(
        model_list, contamination_min_amplitude_list, contamination_frequency_range_list, proba_list))
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for model_version, contamination_min_amplitude, contamination_frequency_range, proba in params_list:

                    false_spindle_params = dict(
                        duration_range=[0.5, 1.5],  # [s]
                        bandstop_cutoff=[8, 17],  # [Hz]
                        spindle_frequency_range=[12, 14.5],  # [Hz]
                        spindle_frequency_variation_width=1,  # [Hz]
                        spindle_amplitude_absolute_range=[7 / dataset.global_std, 25 / dataset.global_std],
                        spindle_amplitude_relative_variation_width=0.3,
                        contamination_frequency_range=contamination_frequency_range,  # [Hz]  IMPORTANT
                        contamination_frequency_variation_width=1,  # [Hz]
                        contamination_amplitude_relative_range=[contamination_min_amplitude, 1.1],  # IMPORTANT
                        contamination_amplitude_relative_variation_width=0.3
                    )

                    params[pkeys.MODEL_VERSION] = model_version
                    params[pkeys.AUG_FALSE_SPINDLES_SINGLE_CONT_PARAMS] = false_spindle_params
                    params[pkeys.AUG_FALSE_SPINDLES_SINGLE_CONT_PROBA] = proba

                    folder_name = '%s_cont%1.1f-%1.1f_min%1.2f_p%1.1f' % (
                        model_version,
                        contamination_frequency_range[0], contamination_frequency_range[1],
                        contamination_min_amplitude, proba)
