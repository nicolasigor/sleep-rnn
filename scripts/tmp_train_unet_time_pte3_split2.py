...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'unet_time_pte3'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    init_filters_list = [32, 16]
    n_pools_list = [5, 4, 3]
    down_factor_list = [1, 2, 4]
    focusing_param_list = [2.0, 1.0]

    param_list = list(itertools.product(
        model_version_list, init_filters_list, n_pools_list,
        down_factor_list, focusing_param_list))
    ...
    # Selected params
    params[pkeys.UNET_TIME_N_CONV_DOWN] = 1
    params[pkeys.UNET_TIME_N_CONV_UP] = 1
    params[pkeys.TYPE_LOSS] = constants.FOCAL_LOSS
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for model_version, init_filters, n_pools, down_factor, focusing_param in param_list:

                    params[pkeys.MODEL_VERSION] = model_version
                    params[pkeys.UNET_TIME_INITIAL_CONV_FILTERS] = init_filters
                    params[pkeys.UNET_TIME_N_DOWN] = n_pools
                    params[pkeys.TOTAL_DOWNSAMPLING_FACTOR] = down_factor
                    params[pkeys.FOCUSING_PARAMETER] = focusing_param

                    folder_name = '%s_f_%d_pools_%d_downFactor_%d_gamma_%1.1f' % (
                        model_version, init_filters, n_pools, down_factor, focusing_param)
