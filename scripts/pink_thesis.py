...
if __name__ == '__main__':
    ...
    for source_config in source_configs:
        ...
        # set the proper event type in pink
        pink.event_name = source_dataset.event_name

        experiment_name = '%s_from_%s_desc_pink_to' % (this_date, source_config["ckpt_folder"])
        experiment_name_full = '%s_e%d_%s_train_%s' % (experiment_name, 1, task_mode, pink.dataset_name)
        ...
        for grid_folder in grid_folder_list:
            ...
            for fold_id in range(len(partitions['test'])):
                ...
                pink.global_std = configs_std[get_hash_name(source_config)][fold_id]
                # Create data feeders
                data_test = FeederDataset(pink, pink.all_ids, task_mode, which_expert=1)
                # Retrieve appropriate model
                source_path = os.path.join(RESULTS_PATH, grid_folder_complete, 'fold%d' % fold_id)
                model = get_model(source_path, source_config["dataset_name"])
                ...
                for perturbation_name, perturbation_args in perturbations:
                    ...
                    save_dir = os.path.abspath(os.path.join(
                        RESULTS_PATH, 'predictions_%s' % pink.dataset_name, base_dir))
    ...
    for source_config in source_configs:
        ...
        experiment_name_full = '%s_e%d_%s_train_%s' % (
            experiment_name, 1, task_mode, pink.dataset_name)
        grid_folder_list = os.listdir(os.path.join(RESULTS_PATH, source_config["ckpt_folder"]))
        for grid_folder in grid_folder_list:
            grid_folder_complete = os.path.join(source_config["ckpt_folder"], grid_folder)
