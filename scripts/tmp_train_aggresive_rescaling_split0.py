...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'aggresive_rescaling'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    rescale_unif_normal_proba_list = [
        (1.0, 0.0),
        (0.0, 1.0)
    ]
    params_list = list(itertools.product(model_list, rescale_unif_normal_proba_list))

    # Base parameters
    params = pkeys.default_params.copy()
    params[pkeys.AUG_RESCALE_UNIFORM_INTENSITY] = 0.5
    params[pkeys.AUG_RESCALE_NORMAL_STD] = 0.3
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for model_version, rescale_unif_normal_proba in params_list:

                    unif_proba = rescale_unif_normal_proba[0]
                    normal_proba = rescale_unif_normal_proba[1]

                    params[pkeys.MODEL_VERSION] = model_version
                    params[pkeys.AUG_RESCALE_UNIFORM_PROBA] = unif_proba
                    params[pkeys.AUG_RESCALE_NORMAL_PROBA] = normal_proba

                    folder_name = '%s_unif%d_normal%d' % (model_version, unif_proba, normal_proba)
