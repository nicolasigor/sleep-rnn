...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'dilated_mkconv'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    params_list = list(itertools.product(
        model_list,
        conv_skip_list, kernel_size_list,
        init_filters_list, first_block_behavior_list
    ))
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for model_version, conv_skip, kernel_size, init_filters, first_block_behavior in params_list:
                    first_strat = 'none' if first_block_behavior == 'single' else 'dilated'
                    mkd_filters_1 = generate_mkd_specs(first_strat, kernel_size, init_filters)
                    mkd_filters_2 = generate_mkd_specs('dilated', kernel_size, 2 * init_filters)
                    mkd_filters_3 = generate_mkd_specs('dilated', kernel_size, 4 * init_filters)

                    params[pkeys.MODEL_VERSION] = model_version
                    params[pkeys.TIME_CONV_MKD_FILTERS_1] = mkd_filters_1
                    params[pkeys.TIME_CONV_MKD_FILTERS_2] = mkd_filters_2
                    params[pkeys.TIME_CONV_MKD_FILTERS_3] = mkd_filters_3
                    params[pkeys.TIME_CONV_MK_SKIPS] = conv_skip

                    folder_name = '%s_skip%d_k%d_f%d_first-%s' % (
                        model_version,
                        conv_skip, kernel_size,
                        init_filters, first_block_behavior)
