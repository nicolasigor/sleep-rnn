...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'border_weights'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    description_str = 'v11 with weights at borders'
    verbose = True
    ...
    # Grid parameters
    amplitude_list = [2, 4, 8, 16]
    half_width_list = [1, 2, 4, 6, 8, 10, 12]
    type_loss_list = [
    ...
    params_list = list(itertools.product(
        type_loss_list, amplitude_list, half_width_list
    ))
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for type_loss, amplitude, half_width in params_list:
                    model_version = params[pkeys.MODEL_VERSION]

                    params[pkeys.TYPE_LOSS] = type_loss
                    params[pkeys.BORDER_WEIGHT_AMPLITUDE] = amplitude
                    params[pkeys.BORDER_WEIGHT_HALF_WIDTH] = half_width

                    if type_loss == constants.CROSS_ENTROPY_BORDERS_IND_LOSS:
                        short_loss_name = 'xent_borders_ind'
                    else:
                        short_loss_name = 'xent_borders'

                    folder_name = '%s_%s_a%02d_hw%02d' % (
                        model_version, short_loss_name,
