...
if __name__ == '__main__':
    ...
    # ----- Experiment settings
    experiment_name = 'elastic_grid_pte4'
    task_mode_list = [
        constants.N2_RECORD
    ]
    ...
    description_str = 'elastic grid search, ablation'
    which_expert = 1
    verbose = True
    ...
    elastic_proba_list = [0.5, 0.0]
    noise_proba_list = [0.5, 0.0]
    drop_rate_before_lstm_list = [0.5, 0.2]

    param_list = list(itertools.product(
        model_version_list, elastic_proba_list, noise_proba_list, drop_rate_before_lstm_list))
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for model_version, elastic_proba, noise_proba, drop_rate_before in param_list:

                    params[pkeys.AUG_ELASTIC_PROBA] = elastic_proba
                    params[pkeys.AUG_GAUSSIAN_NOISE_PROBA] = noise_proba
                    params[pkeys.DROP_RATE_BEFORE_LSTM] = drop_rate_before
                    params[pkeys.MODEL_VERSION] = model_version

                    folder_name = '%s_elastic_%1.1f_noise_%1.1f_drop_%1.1f' % (
                        model_version, elastic_proba, noise_proba, drop_rate_before)
