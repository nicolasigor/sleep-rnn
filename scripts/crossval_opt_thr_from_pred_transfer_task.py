...
if __name__ == '__main__':
    ...
    source_task_mode = constants.WN_RECORD
    ...
    full_ckpt_folder = '%s_%s_train_%s' \
                       % (ckpt_folder, source_task_mode, dataset_name)
        ...
        for k in id_try_list:
            ...
            for set_name in set_list:
                ...
                filename = os.path.join(
                        ckpt_path,
                        'prediction_%s_%s.pkl' % (source_task_mode, set_name))
    ...
    # Search optimum
    print('\nVal AF1 report for %s (Transfer from %s to %s)' % (
        full_ckpt_folder, source_task_mode, target_task_mode))
    ...
    for j, folder_name in enumerate(grid_folder_list):
        ...
        for k in id_try_list:
            ...
            data_val = FeederDataset(
                dataset, val_ids, target_task_mode, which_expert=which_expert)
            data_train = FeederDataset(
                dataset, train_ids, target_task_mode, which_expert=which_expert)
            ...
            prediction_obj_val.task_mode = target_task_mode
            prediction_obj_train.task_mode = target_task_mode
