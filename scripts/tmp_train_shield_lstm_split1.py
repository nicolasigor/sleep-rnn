...
if __name__ == '__main__':
    ...
    description_str = 'shielding lstm to speed up computation'
    verbose = True
    ...
    # Grid parameters
    shield_down_list = [10, 5, 4, 2, 1]
    shield_pool_list = [constants.AVGPOOL, constants.MAXPOOL]
    params_list = list(itertools.product(
        shield_down_list, shield_pool_list
    ))

    # Base parameters
    params = pkeys.default_params.copy()
    params[pkeys.MODEL_VERSION] = constants.V11_SHIELD
    params[pkeys.FC_UNITS_1] = 512
    ...
    for task_mode in task_mode_list:
        for dataset_name in dataset_name_list:
            ...
            for id_try in id_try_list:
                ...
                for shield_down, shield_pool in params_list:
                    model_version = params[pkeys.MODEL_VERSION]
                    params[pkeys.SHIELD_LSTM_DOWN_FACTOR] = shield_down
                    params[pkeys.SHIELD_LSTM_TYPE_POOL] = shield_pool

                    folder_name = '%s_%d_%s' % (
                        model_version, shield_down, shield_pool)
