...
if __name__ == '__main__':
    ...
    this_date = '20210705'  # datetime.datetime.now().strftime("%Y%m%d")
    ...
    for target_config in target_configs:
        for source_config in source_configs:
            source_grid_folder_list = os.listdir(os.path.join(RESULTS_PATH, source_config["ckpt_folder"]))
            for source_grid_folder in source_grid_folder_list:
                source_grid_folder_complete = os.path.join(source_config["ckpt_folder"], source_grid_folder)
                optimal_thr_list = OPTIMAL_THR_FOR_CKPT_DICT[source_grid_folder_complete]
                ...
                opt_thr_str = get_opt_thr_str(optimal_thr_list * 3, experiment_name_full, source_grid_folder)
                print(opt_thr_str)
