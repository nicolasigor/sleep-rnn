...
class MassSSAlt(Dataset):
    ...
    def __init__(self, params=None, load_checkpoint=False, verbose=True, custom_scaling_dict=None):
        ...
        self.reference_channel = 'EEG A2-CLE'  # Channel to reference, if possible
    ...
    def _read_eeg(self, path_eeg_file):
        ...
        with pyedflib.EdfReader(path_eeg_file) as file:
            ...
            # Re-reference if possible
            if self.reference_channel in channel_names:
                reference_to_extract = channel_names.index(self.reference_channel)
                reference = file.readSignal(reference_to_extract)
                signal = signal - reference
                # Check
                print('Channel extracted: (%s) - (%s)' % (
                    file.getLabel(channel_to_extract), file.getLabel(reference_to_extract)))
            else:
