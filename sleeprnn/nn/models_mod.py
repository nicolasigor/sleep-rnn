...
from .base_model_mod import BaseModelMod
...
class WaveletBLSTMMod(BaseModelMod):
    ...
    def fit(
        ...
        for single_x, single_id in zip(x_val, ids_val):
            self._update_cwt_stats(single_x, single_id)
        ...
        # prepare sub_ids "labels"
        sub_ids_train = []
        for single_x, single_id in zip(x_train, ids_train):
            n_segments = single_x.shape[0]
            this_ids = [single_id] * n_segments
            this_ids = np.asarray(this_ids)
            sub_ids_train.append(this_ids)
        ...
        for single_x, single_id in zip(x_val, ids_val):
            ...
            this_ids = [single_id] * n_segments
            this_ids = np.asarray(this_ids)
            sub_ids_val.append(this_ids)

        # Transform to numpy arrays
        x_train = np.concatenate(x_train, axis=0)
        y_train = np.concatenate(y_train, axis=0)
        sub_ids_train = np.concatenate(sub_ids_train, axis=0)
        x_val = np.concatenate(x_val, axis=0)
        y_val = np.concatenate(y_val, axis=0)
        sub_ids_val = np.concatenate(sub_ids_val, axis=0)

        # Shuffle training set
        x_train, y_train, sub_ids_train = utils.shuffle_data_with_ids(
            x_train, y_train, sub_ids_train, seed=0)

        print('Training set shape', x_train.shape, y_train.shape, sub_ids_train.shape)
        print('Validation set shape', x_val.shape, y_val.shape, sub_ids_val.shape)
        ...
        x_train_1, y_train_1, sub_ids_train_1, x_train_2, y_train_2, sub_ids_train_2 = self._split_train(
            x_train, y_train, sub_ids_train)
        self._init_iterator_train(x_train_1, y_train_1, sub_ids_train_1, x_train_2, y_train_2, sub_ids_train_2)
        ...
        val_loss, val_metrics, _ = self.evaluate(x_val, y_val, sub_ids_val)
        last_model = {
            KEY_ITER: niters,
    ...
    def _split_train(self, x_train, y_train, sub_ids_train):
        ...
        if n_with_activity < n_train / 2:
            ...
            sub_ids_train_1 = sub_ids_train[zero_activity_idx]
            # Pages with activity
            x_train_2 = x_train[exists_activity_idx]
            y_train_2 = y_train[exists_activity_idx]
            sub_ids_train_2 = sub_ids_train[exists_activity_idx]
