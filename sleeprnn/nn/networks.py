...
def wavelet_blstm_net_v11_multi(
        inputs,
        params,
        training,
        name='model_v11_multi'
):
    print('Using model V11 Multilabel (Time-Domain)')
    with tf.variable_scope(name):
...
def wavelet_blstm_net_v19_multi(
        inputs,
        params,
        training,
        name='model_v19_multi'
):
    print('Using model V19 multilabel (general cwt)')
    with tf.variable_scope(name):
        # CWT stage
        border_crop = int(
            params[pkeys.BORDER_DURATION] * params[pkeys.FS])
