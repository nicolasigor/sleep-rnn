...
class MASS(object):
    ...
    def get_file_paths(self):
        ...
        for i, reg_id in enumerate(self.reg_ids):
            path_eeg_file = os.path.join(
                PATH_MASS, PATH_REC, '01-02-%04d PSG.edf' % reg_id)
            path_states_file = os.path.join(
                PATH_MASS, PATH_STATES, '01-02-%04d Base.edf' % reg_id)
            path_marks_1_file = os.path.join(
                PATH_MASS, PATH_MARKS_1, '01-02-%04d SpindleE1.edf' % reg_id)
            path_marks_2_file = os.path.join(
                PATH_MASS, PATH_MARKS_2, '01-02-%04d SpindleE2.edf' % reg_id)
            # Save paths
            ind_dict = {
    ...
    def save_checkpoint(self):
        ...
        filename = os.path.join(
            PATH_MASS, PATH_CHECKPOINT, '%s.pickle' % self.name)
    ...
    def load_from_files(self):
        ...
        for i, path_dict in enumerate(data_path_list):
            ...
            n2_pages = self.read_states(path_dict[KEY_FILE_STATES], signal_len)
            print('N2 pages: %d' % n2_pages.shape[0])
            signal = norm_clip_eeg(signal, n2_pages, self.page_size)
            marks_1 = self.read_marks(
                path_dict['%s_1' % KEY_FILE_MARKS], fs_old)
            marks_2 = self.read_marks(
                path_dict['%s_2' % KEY_FILE_MARKS], fs_old)
            print('Marks from E1: %d, Marks from E2: %d'
                  % (marks_1.shape[0], marks_2.shape[0]))
            marks_1 = inter2seq(marks_1, 0, signal_len-1)  # To binary sequence
            marks_2 = inter2seq(marks_2, 0, signal_len-1)  # To binary sequence
            # Save data
            ind_dict = {
                KEY_EEG: signal,
                KEY_PAGES: n2_pages,
    ...
    def get_subject_data(
            self, reg_id, augmented_page=False, border_size=0, which_mark=1):
        ...
        Args:
            ...
            which_mark: (Optional, {1, 2}, defaults to 1) Whether to get E1
                marks (1) or E2 marks (2).
