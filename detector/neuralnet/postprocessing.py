"""postprocessing.py: Module for general postprocessing operations of predicted
annotations."""
...
def filter_duration_marks(marks, fs, min_duration, max_duration):
    """Removes marks that are too short or too long. Marks are assumed to be
    sample-stamps."""
    durations = (marks[:, 1] - marks[:, 0]) / fs
    feasible_idx = np.where(
        (durations >= min_duration) & (durations <= max_duration))[0]
